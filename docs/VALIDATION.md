...
python -m py_compile gremlinEx.py gremlin/ui/axis_calibration.py mechagremlin/curves.py mechagremlin/studio.py mechagremlin/theme.py
Exit 0
...
Visual review used explicit installed Segoe UI fonts with Qt offscreen rendering at 1180x810 and 780x680. The smaller window scrolls instead of overlapping. This is not a screen-reader audit or full DPI/display matrix.
...
The refined UI pass adds selected/custom preset state and a regression for repeat selection. A separate agent reviewed both rendered layouts and the updated UI source with no blocking findings.
